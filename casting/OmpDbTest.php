	...
	/*$sql='
			...
			$sql='
				SELECT
					CBAS.BAKADSCHD_TRANSACTION_ID,
					CSTB.BNR_SEQUENCE
				FROM COMPCHT15.CS_BARKER_AD_SCHEDULE CBAS
				INNER JOIN COMPCHT15.CS_BARKER_AD CBA ON CBA.BAKAD_RECID=CBAS.BAKAD_RECID
				INNER JOIN COMPCHT15.CS_GROUP_AD CGA ON CBAS.BAKADSCHD_RECID=CGA.BAKADSCHD_RECID
				INNER JOIN COMPCHT15.CS_AD_SCHEDULE CAS ON CAS.AD_SCHDID=CGA.AD_SCHDID
				LEFT OUTER JOIN COMPCHT15.CS_SER_TMP_BNR CSTB ON CSTB.CS_STB_RECID=CAS.CS_STB_RECID
				WHERE
					CBAS.BAKADSCHD_TRANSACTION_ID=:TID
				ORDER BY
					CSTB.BNR_SEQUENCE
