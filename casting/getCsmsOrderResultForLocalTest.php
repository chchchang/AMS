<?php
		...
		if($版位類型名稱 == '首頁banner' || $版位類型名稱 == '專區banner'){
			$sql='
				SELECT
					...
					CAS.ASSIGN_END_TIME,
					CAS.SCHD_STATUS
				FROM '.$DB_U.'.CS_SERVICE CS
				INNER JOIN '.$DB_U.'.CS_SER_TMP_BNR CSTB ON CS.SER_RECID=CSTB.SER_RECID
				INNER JOIN '.$DB_U.'.CS_AD_SCHEDULE CAS ON CSTB.CS_STB_RECID=CAS.CS_STB_RECID
				INNER JOIN '.$DB_U.'.CS_BANNER CB ON CSTB.BNR_RECID=CB.BNR_RECID
				INNER JOIN '.$DB_U.'.CS_AD CA ON CA.AD_RECID=CAS.AD_RECID
				INNER JOIN '.$DB_U.'.CS_LINK LK ON CA.LINK_RECID=LK.LINK_RECID
				WHERE
			...
			$sql='
				SELECT
					...
					CSS.SEPG_STATUS
				FROM '.$DB_U.'.OVA_CHANNEL OC
				INNER JOIN '.$DB_U.'.CS_SEPG_CHAN_RELATION CSCR ON OC.CHAN_RECID=CSCR.CHAN_RECID
				INNER JOIN '.$DB_U.'.CS_SEPG_SCHEDULE CSS ON CSCR.SEPG_SCHDID=CSS.SEPG_SCHDID
				INNER JOIN '.$DB_U.'.CS_AD CA ON CA.AD_RECID=CSS.AD_RECID
				INNER JOIN '.$DB_U.'.CS_LINK LK ON CA.LINK_RECID=LK.LINK_RECID
				WHERE
			...
			$sql='
				SELECT
					...
					CSTB.BNR_SEQUENCE
				FROM '.$DB_U.'.CS_BARKER_AD_SCHEDULE CBAS
				INNER JOIN '.$DB_U.'.CS_SERVICE CS ON CBAS.SER_RECID=CS.SER_RECID
				INNER JOIN '.$DB_U.'.CS_BARKER_AD CBA ON CBA.BAKAD_RECID=CBAS.BAKAD_RECID
				INNER JOIN '.$DB_U.'.CS_LINK LK ON CBA.LINK_RECID=LK.LINK_RECID
				LEFT OUTER JOIN '.$DB_U.'.CS_GROUP_AD CGA ON CBAS.BAKADSCHD_RECID=CGA.BAKADSCHD_RECID
				LEFT OUTER JOIN '.$DB_U.'.CS_AD_SCHEDULE CAS ON CAS.AD_SCHDID=CGA.AD_SCHDID
				LEFT OUTER JOIN '.$DB_U.'.CS_SER_TMP_BNR CSTB ON CSTB.CS_STB_RECID=CAS.CS_STB_RECID
				LEFT OUTER JOIN '.$DB_U.'.OVA_VOD_CONTENT OVCSD ON CBA.SD_VODCNT_RECID=OVCSD.VODCNT_RECID
				LEFT OUTER JOIN '.$DB_U.'.OVA_VOD_CONTENT OVCHD ON CBA.HD_VODCNT_RECID=OVCHD.VODCNT_RECID
				WHERE
	...
	function updateOrderWithSameMaterial($託播單識別碼群組){
		...
		$mutex = new Mutex("CommitOrderToCSMS");
		$mutex->lock();
