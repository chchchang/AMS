...
//function getTableData($sqlparas){
	...
	$DG_header=array('素材識別碼','素材名稱','素材說明','素材原始檔名','影片素材秒數','import_time');
	//取得資料
	$sql = 'SELECT '.join($DG_header,',').' FROM 素材 left JOIN barker_material_import_result ON (素材識別碼 = material_id)
	WHERE 素材類型識別碼=3	
		AND 素材群組識別碼 LIKE ? 
		AND(
		...
		AND ((import_time BETWEEN ? AND ?)'.($sqlparas["CAMPSTimeFlag"]?'':'OR import_time IS NULL').')
		'.($sqlparas["unCimmitOnly"]=='true'?'AND ( import_time IS NULL )':'')
