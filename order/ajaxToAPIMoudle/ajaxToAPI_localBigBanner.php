<?php
	//用於在地專區大BANNER版位託播單，需備/order/ajaxToAPI.php使用，否則無法運作
	require_once('../tool/auth/authAJAX.php');
	require_once('../Config_VSM_Meta.php');
	define("APIUrl",Config::GET_API_SERVER_852_LOCALBIGBANNER());
	

	function sendOrder_localBigBanner($orderId){
		global $logger, $my;
		...
		if(!$apiResult=connec_to_Api_json(APIUrl,'POST',$postvars)){
			recordResult($rcordaction,0,"無法連接大BANNER廣告API",null);
			$logger->error('無法連大BANNER廣告API:'.APIUrl);
			exit(json_encode(array("success"=>false,"message"=>'無法連接大BANNER廣告API','id'=>$orderId),JSON_UNESCAPED_UNICODE));	
		}
			...
			foreach($checkResult['data'] as $mresult){
				if(!$mresult["action_status"]){
					...
					if(!$apiResult=connec_to_Api_json(APIUrl,'POST',$postvars)){
						...
						$logger->error('無法連奧運廣告API:'.APIUrl);
						exit(json_encode(array("success"=>false,"message"=>'無法連接大BANNER廣告API','id'=>$orderId),JSON_UNESCAPED_UNICODE));	
					}
					$checkResult = json_decode($apiResult,true);
					if($checkResult['success']){
	...
	function cancelOrder_localBigBanner($orderId){
		global $logger, $my;
		$action = "delete";
		$bypostOrder = get_order_post_data($orderId,$action);		
	...
	function connec_to_Api_json($url,$method,$postvars){
		...
		//print_r($apiResult);
		return $apiResult;
		/*$test = array("success"=>true,"errormsg"=>$postvars);
		return $test;*/
	}
