...
<script type="text/javascript">
	...
	function showPositionList(){
		...
		$.post(ajaxtodbPath,bypost,function(json){
				json.header.push('新增版位');
				for(var row in json.data){
					json.data[row].push(['新增版位','button']);
				...
				DG.buttonCellOnClick=function(y,x,row) {
					if(row[x][0]=='新增版位') {
						$("#dialog_iframe").attr("src","positionTypeForm.php?positionPage=1&id="+row[0][0])
						.css({"width":"100%","height":"100%"}); 
						dialog=$( "#dialog_form" ).dialog({height: 700, width:900, title:"新增版位"});
						dialog.dialog( "open" );
					}
				}
